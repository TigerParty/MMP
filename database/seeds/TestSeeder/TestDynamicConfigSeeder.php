...
class TestDynamicConfigSeeder extends Seeder {
	public function run() {
		$records = array(
            array('key' => 'header_title', 'value' => 'Argo Wantok Testing Environment', 'cache_enabled' => 1),
		...
		$this->command->info("dynamic_config : Updating " . count($records) . " records...");
		foreach($records as $record) {
			DB::table('dynamic_config')->where('key', $record['key'])->update(['value' => $record['value']]);
		}
	}
}